...
Авторизація на сайті + Вихід з Web Сайту та Перевірка
...
def test_case_logout():
    ...
    # Вибір зі списку меню вихід
    WebDriverWait(driver, 20).until(EC.element_to_be_clickable((By.XPATH, "/html/body/qa-root/qa-dashboard/qa-header/div/div[2]/div/button[2]/span/span"))).click()
    WebDriverWait(driver, 20).until(EC.element_to_be_clickable((By.XPATH, "//*[@id=\"cdk-overlay-0\"]/div/div/button"))).click()

    time.sleep(5)
    # Отримаемо значення з сторінки singin:
    text_all_singin_check = driver.find_element("xpath", "/html/body/qa-root/qa-sign-in/div/div/div[1]/div/h2")

    prooftextchange = text_all_singin_check.text
    # Тестування:
    check.equal(prooftextchange, "Sign In")

    time.sleep(5)

    if text_all_singin_check is None:
        print("Елемент не знайдено!")
        time.sleep(5)
    else:
        print(str(text_all_singin_check.text))
        print("Елемент знайдено!")
        time.sleep(5)
